...
def configure_ngrok():
    ngrok_token = "2pR1IT2GuwwRF5X5L44btEWmcEZ_3xdVtr4kDBSyufgUwMEUU"
    try:
...
def update_soul_txt(public_url):
    with open("anony7.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in anony7.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony7.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony7.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony7.txt on VPS: {str(e)}")
